...
for grp in ${grp_list}; do
   echo -e "${grp}\n" |$gmx_pth/gmx_d rmsf -f  ${traj} -s ${ref_struc} -b $Tskip  -n ${indexfile} -res -aniso \
       -ox aver_${Tskip}ps-end_res_${grp}.pdb -oq bfac_${Tskip}ps-end_res_${grp}.pdb -o rmsf_${Tskip}ps-end_res_${grp}.xvg -od rmsd_${Tskip}ps-end_res_${grp}.xvg
done
...
def draw_bar_per_residue(slGroups, tskip, val_title, bar_color):
    NumGrps = len(slGroups)
    ilDrawGrpIdxs = range(NumGrps) 
    
    fig_subpl, ax = plt.subplots(nrows=NumGrps, ncols=1, sharex=False,figsize=(10,15))
    fig_subpl.suptitle(val_title.upper() + ' of Pocket Residues', fontsize=20, fontweight='bold')
    fig_subpl.supxlabel('Residue Number',fontsize=20, fontweight='bold',y=0.0)
    fig_subpl.supylabel(val_title.upper() + ', (\u212B)',fontsize=20, fontweight='bold',x=0.0)
    ...
    for drawGrp in slGroups:
        ...
        for line in open(xvg_file, 'r'):
            ...
            X.append(values[0])   # * 1000 to Convert Time units ps->ns
            Y.append(values[1] * 10)
        print('Finish reading XVG ', xvg_file)
        Xsorted = [str(int(i) + $resShift) for i in X]
        Ysorted = Y
        ...
        axis.bar(Xsorted, Ysorted, color=bar_color, label=drawGrp)
        axis.set_xticks(np.arange(len(Xsorted))) # Set tick locations
        axis.set_xticklabels(Xsorted, fontsize=16) # Set tick labels
        axis.tick_params(axis='both', which='major', labelsize=16)

        axis.set_title(drawGrp, fontsize=20, loc='center', y=1.00, fontweight='bold')
        axis.set_ylim(ymin=0,ymax="${yMax}")
        iGrp = iGrp + 1
    
    xtick_major_spacing=10
    xtick_minor_spacing=5
...
tskip = '${Tskip}'
draw_bar_per_residue(slGroups, tskip, 'rmsd', 'blue')
draw_bar_per_residue(slGroups, tskip, 'rmsf', 'blue')
