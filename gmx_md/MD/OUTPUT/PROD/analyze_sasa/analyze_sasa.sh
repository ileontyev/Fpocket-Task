...
for grp in ${grp_list}; do
   $gmx_pth/gmx_d sasa  -surface Protein -output ${grp} -f ${traj} -s ${ref_struc} -b $Tskip  -n ${indexfile} \
       -o sasa_${Tskip}ps-end_res_${grp}.xvg -tv volume_${Tskip}ps-end_res_${grp}.xvg
done


outfnm="sasa_analysis.out"
...
def draw_bar_per_residue(slGroups, tskip, val_title, bar_color):
    ...
    fig_subpl.supylabel('SASA, (\u212B^2)',fontsize=18, fontweight='bold',x=0.005)
    ...
    for drawGrp in slGroups:
        ...
        axis.plot(X, Y, color='blue',linewidth=1.0, label=slGroupsRaw[ilDrawGrpIdxs[iGrp]])
        ...
        axis.axhline(y=av[iGrp], color='b', linestyle='-', linewidth=0.7, label='mean value')
...
tskip = '${Tskip}'
draw_bar_per_residue(slGroups, tskip, 'sasa', 'blue')
"
